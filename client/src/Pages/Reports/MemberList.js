...
const MemberList = () => {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_URL}/getmember/${selectedDate}`);
      setMember(res.data);
  ...
  useEffect(() => {
    // getData();  //calling from useeffect on bases of selectedDate
    getClan();
    // handleDebit();
  }, []);
  ...
  const searchFilter = async () => {
    ...
    const res = await axios.post(`${process.env.REACT_APP_URL}/getMemberListByFilter`, clanData);
    console.log(res.data)
    if (res.data) {
      setMember(res.data);
    } else {
