...
const PaymentHistory = () => {
  ...
  const getPaymentHistory = async () => {
    try {
      await axios.get(`${process.env.REACT_APP_URL}/getPaymentHistory`)
        .then((res) => {
          //  console.log(res)
  ...
  useEffect(() => {
    ...
    const filterMembers = () => {
      return paymentHistory.filter((k) => {


        const group = `${k.f_name} ${k.m_name} ${k.l_name}`
        const lowercaseSearchFilter = searchFilter.toLowerCase();
        const lowercaseFName = k.f_name.toLowerCase();
        const lowercaseMName = k.m_name.toLowerCase();
        const lowercaseLName = k.l_name.toLowerCase();
        const lowercaseGroup = group.toLowerCase();


        return (
          lowercaseSearchFilter === "" ||
          lowercaseFName.includes(lowercaseSearchFilter) ||
          lowercaseMName.includes(lowercaseSearchFilter) ||
          lowercaseLName.includes(lowercaseSearchFilter) ||
          lowercaseGroup.includes(lowercaseSearchFilter) ||
          Number(searchFilter) === k.id ||
          Number(searchFilter) === k.member_id ||
          Number(searchFilter) === k.pay_amount ||
          Number(searchFilter) === k.book_no ||
          Number(searchFilter) === k.voucher_no
        );
      })
  };
  ...
  useEffect(() => {
      setfilteredPayments(filterMembers())
  ...
  const numberOfPage = Math.ceil(filteredPayments.length / itemPerPage);
