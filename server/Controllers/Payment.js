...
const getSingalPaymentData = (req, res) => {
    const query = "SELECT c.clan_name, CONCAT(m.f_name, ' ', m.m_name, ' ', m.l_name) AS member_name, p.member_id, p.pay_amount FROM payment p JOIN member m ON p.member_id = m.id JOIN clan c ON m.clanid = c.id limit 1";
...
module.exports = { getPaymentHistory, getPaymentInfo, updatePayment, deletePayment, getSingalPaymentData }